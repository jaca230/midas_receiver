...
void process_event(HNDLE hBuf, HNDLE request_id, EVENT_HEADER * pheader, void *pevent)
{
   ...
   static int first_event = 1;  // To track the first event
   ...
   /* I'm 95% sure this is broken, it's almost never the case that 
      pdata[0] != size || pdata[size / 4 - 1] != size
      is false
   */
   ...
   if (!jam) {
      /* only test once */
      pdata = (INT *) (pheader + 1);
      if ((pdata[0] != size || pdata[size / 4 - 1] != size) && !first_event)
   ...
   if (all_flag) {
      // Skip serial number mismatch check for the first event or if this is the first event of a new run
      if (first_event || pheader->serial_number == 0) {
         first_event = 0;  // After the first event, set this to false
      } else if ((INT) pheader->serial_number != ser[id] + 1) {
...
int main()
{
  ...
  // printf("Host to connect: ");
   ...
   //Note you can also get host name and expt name from the environment:
   ...
   bm_open_buffer("BUF001", 2*MAX_EVENT_SIZE, &hBufEvent);

   /* set the buffer cache size */
   bm_set_cache_size(hBufEvent, 100000, 0);

   /* place a request for a specific event id */
   bm_request_event(hBufEvent, (WORD) EVENTID_ALL, TRIGGER_ALL,
