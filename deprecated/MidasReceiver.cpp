...
// Constructor
MidasReceiver::MidasReceiver(int id, 
    ...
    : id(id), 
      hostName(host), 
      bufferName(bufferName.empty() ? "SYSTEM" : bufferName), 
      clientName(clientName.empty() ? "Event Receiver" : clientName), 
      eventID(eventID), 
...
void MidasReceiver::run() {
    ...
    status = bm_request_event(hBufEvent, (WORD)eventID, TRIGGER_ALL, getAllEvents ? GET_ALL : GET_NONBLOCKING,
                              &requestID, MidasReceiverManager::processEventCallback); // Update status when event request is made
    ...
    status = cm_msg_register(MidasReceiverManager::processMessageCallback);

    if (status != CM_SUCCESS) {
        cm_msg(MERROR, "MidasReceiver::run", "Failed to request message. Status: %d", status);
        listeningForEvents = false;
        return;
    }

   /* place a request for transition notification */
    status = cm_register_transition(TR_START, MidasReceiverManager::processTransitionCallback, 500);

    /*
    @param transition Transition to register for (see @ref state_transition)
    @param func Callback function.
    @param sequence_number Sequence number for that transition (1..1000) <-- Will need to be a user input, 500 is right between MasterGM2 (499) and AMC13001 (501)
    @return CM_SUCCESS
    INT cm_register_transition(INT transition, INT(*func)(INT, char *), INT sequence_number) {


    NOT SURE IF THIS SHOULD BE A USER INPUT:
    ...
    */
    ...
    if (status != CM_SUCCESS) {
        cm_msg(MERROR, "MidasReceiver::run", "Failed to request transition. Status: %d", status);
        listeningForEvents = false;
        return;
    }
